...
gulp.task('copy', function() {
    return gulp.src('src/framework/**/*')
        .pipe(gulp.dest(buildBasePath+'framework'));
});
...
//css
gulp.task('minifycss', ["LessToCss","AutoFx"],function (){
    return gulp.src('src/cssTemp/**/*.css')
...
gulp.task('replacejs', function(){
    return gulp.src([buildBasePath+'*.html'])
        .pipe(replace('main.js', 'build.min.js'))
        .pipe(gulp.dest(buildBasePath));
});
//生产使用，替换文件名，common.css替换为build.min.css
gulp.task('replacecss', function(){
    return gulp.src([buildBasePath+'*.html'])
        .pipe(replace('main.css', 'build.min.css'))
        .pipe(gulp.dest(buildBasePath));
});
//开发使用，替换文件名，main.js替换为build.js
...
gulp.task('replacecssdev', function(){
    return gulp.src([buildBasePath+'*.html'])
        .pipe(replace('main.css', 'build.css'))
        .pipe(gulp.dest(buildBasePath));
});
...
gulp.task('rev', function() {
    //html，针对js,css,img
    return gulp.src(['src/rev/**/*.json', buildBasePath+'**/*.html'])
        .pipe(revCollector({replaceReved:true }))
        .pipe(gulp.dest(buildBasePath));
});
gulp.task('revimg', function() {
    //css，主要是针对img替换
    return gulp.src(['src/rev/**/rev-img-manifest.json', buildBasePath+'css/*.css'])
        .pipe(revCollector({replaceReved:true }))
...
gulp.task('watch2', function () {
    gulp.watch('src/**/*.html', ['default2']);
});
//监视文件的变化，有修改时，自动调用defaultdev缺省默认任务
gulp.task('watchdev', function () {
    gulp.watch('src/**/*.html', ['defaultdev']);
});
 ...
 * */
gulp.task('defaultdev',function(cb){gulpSequence('clean:Build','copy','copyimg','minifyjs','minifycss','html','replacejsdev','replacecssdev','webserver')(cb);});